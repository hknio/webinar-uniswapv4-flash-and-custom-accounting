...
import {UniswapV4DeltasContract} from "../contracts/UniswapV4DeltasContract.sol";
...
contract UniswapV4DeltasContractTest is Test, Deployers {
    using PoolIdLibrary for PoolKey;
    using StateLibrary for IPoolManager;

    // PoolManager public manager;
    UniswapV4DeltasContract public uniswapV4DeltasContract;
    // PoolKey public key;
    ...
    function setUp() public {
        ...
        uniswapV4DeltasContract = new UniswapV4DeltasContract(address(manager));
    ...
    function testDeltasPerformAddLiquidity() public {
        ...
        token0.approve(address(uniswapV4DeltasContract), initialAmount);
        token1.approve(address(uniswapV4DeltasContract), initialAmount);

        vm.prank(address(uniswapV4DeltasContract));
        token0.approve(address(manager), type(uint256).max);
        vm.prank(address(uniswapV4DeltasContract));
        token1.approve(address(manager), type(uint256).max);

        uniswapV4DeltasContract.addLiquidity(key, tickLower, tickUpper, int256(initialAmount));
    ...
    function testDeltasPerformRemoveLiquidity() public {
        address user = address(this);
        uint256 initialAmount = 1e18;
        ...
        uniswapV4DeltasContract.removeLiquidity(
            key,
            tickLower,
            tickUpper,
    ...
    function testDeltasAddLiquidityAndSwap() public {
        address user = address(this);
        ...
        token0.approve(address(uniswapV4DeltasContract), liquidityAmount);
        token1.approve(address(uniswapV4DeltasContract), liquidityAmount);
        ...
        uniswapV4DeltasContract.addLiquidityAndSwap(
            key,
            tickLower,
            tickUpper,
